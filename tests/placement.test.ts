...
import { PANEL_TOP_FRACTION, panelTop } from '../src/shared/placement'
import { FIX_ACTIONS } from '../src/platform/fix/actions'
import {
  formatHotkey,
  HOTKEY_PRESETS,
  parseHotkey,
...
describe('the hotkey', () => {
  ...
  it('puts the modifiers in one order, so two spellings compare equal', () => {
    expect(parseHotkey('Shift+Ctrl+Super+K')).toEqual(parseHotkey('Super+Ctrl+Shift+K'))
  })
