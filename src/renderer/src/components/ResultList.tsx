...
export function ResultList({ items, selectedIndex, onActivate }: ResultListProps): React.JSX.Element {
  const selected = useRef<HTMLDivElement>(null)

  useEffect(() => {
    // Once the panel has hit its ceiling the list scrolls, so the selection can
    // leave the viewport while the search field stays put.
        ...
        >
          ...
          <span className="result__text">
            ...
            {item.subtitle !== undefined && (
              <span className="result__subtitle">{item.subtitle}</span>
            )}
          </span>
          {index === selectedIndex && (
            <span className="result__action">{ACTION_LABELS[item.kind]}</span>
          )}
        </div>
      ))}
    </div>
