...
/**
 * `Icon` — all 478 members of the spec enum.
 *
 * **Generated from `@raycast/api` 1.104.24, not written by hand.** The list is
 * long, flat and entirely arbitrary, which makes it exactly the kind of thing a
 * person transcribes with a typo nobody notices until an extension renders a
 * blank row. Regenerate it when the spec is bumped rather than editing it here.
 *
 * The **values** are Raycast's own asset names (`"add-person-16"`), kept verbatim
 * because extensions compare against them, store them, and put them in JSON. What
 * we actually draw for each one is the renderer's business (`ext/icons.tsx`).
 *
 * In `shared/` rather than in the api-shim because **both sides need it**: the
 * shim exports it to extensions, and the renderer's glyph table is keyed by these
 * members so it cannot drift from them. `Icon.Gear` is `"cog-16"` and `Icon.Bubble`
 * is `"speech-bubble-16"` — exactly the sort of thing a hand-written second copy
 * gets wrong silently, which is how an accessory ends up as a placeholder dot.
 * Pure data, no imports; safe for `tsconfig.web.json`.
 */
export const Icon = {
...
/** The enum's value type, which is what every `icon` prop accepts. */
